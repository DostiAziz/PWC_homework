...
from pwc_support.storage.retail_repositories import ProductRepository, RefundRepository, ReturnRepository
...
def _db(tmp_path: Path) -> Database:
    ...
    with db.connect() as c:
        ...
        c.execute("INSERT INTO orders VALUES (?,?,?,?,?,?)", ("O-1", "C-1", "delivered", "120", "EUR", now))
...
def test_refund_proposal_requires_expected_version_for_status_change(tmp_path: Path) -> None:
    ...
    request, eligibility = request_return(returns, order_id="O-1", item_id="I-1", reason="size", idempotency_key="k")
