...
def build_retail_tools(
    ...
    def search_products(
        ...
        if len(query) > 200 or (
            max_price is not None and (not math.isfinite(max_price) or max_price < 0)
        ):
            return {
                "products": [],
                "count": 0,
        ...
        rows = products.search(
            query, category, Decimal(str(max_price)) if max_price is not None else None
        )
        return {
            "products": [row.model_dump(mode="json") for row in rows],
            "count": len(rows),
            "source": "products",
        }
    ...
    def check_inventory(product_id: str, location: str | None = None) -> dict[str, object]:
        ...
        return {
            "product_id": product_id,
            "location": location,
            "quantity": products.inventory(product_id, location),
            "source": "inventory",
        }
    ...
    def evaluate_product_return(order_id: str, item_id: str, reason: str) -> dict[str, object]:
        ...
        if (
            not re.fullmatch(r"ORD-[A-Z0-9-]{1,64}", order_id, re.IGNORECASE)
            or not re.fullmatch(r"ITEM-[A-Z0-9-]{1,64}", item_id, re.IGNORECASE)
            or not reason.strip()
            or len(reason) > 1000
        ):
